...
for i=2:note_num
    ...
    note_stpy(i-1,:) = 2*note_diff(i-1,:)/(all_notes_matrix(i,:) + all_notes_matrix(i-1,:));
end
...
if (fig_on)
    ...
    for i =1:var_num
        ...
        T = 1:1:(note_num-1);
        T = T';  
        plot(T, note_stpy(:,i), 'o'), xlabel(['note pair']), ylabel(['relative difference']), title(note_labels(i,:));
