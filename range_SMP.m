function [T_Pxx, X_Pxx, T_SNR, X_SNR, Noise_Pxx, stim_labels, F] = range(file, stim_starts, stim_ends, samp_rate)

% function [T_Pxx, X_Pxx, T_SNR, X_SNR, T_stats, X_stats, Noise_Pxx, stim_labels] = range(file, stim_starts, stim_ends, samp_rate)
%
...
% changed manually within the code to try other values. The defaults we
% began with were those we used in msr_all:
%
%  threshold = 8; 
%  reset = 10;
%  time_code = 'note';
...
% frequency bin. The order of stimuli is as in Pxx. The *_stats variables
% are only calculated for the natural songs (not for the tone or noise).
% Each variable in the *_stats matrices is a different row, with each
% stimulus (eg T1, T2, T3, T4) a different column.
%
% S. Phelps, Feb. 2011
