...
%New commentary from EMG, 2017: tweaked this to work more similary to
%make_mouse_noise in that it now returns ONLY a pure tone file without
%overwriting; also tweaked so that those tone files are returned as a
%2000000x1 double rather than a 1x2000000 double as was previously
%happening. (This is so that the output plays nicer with routines like view_modified_songs.

%If you want it back the way it was, it should be easy to
%uncomment; I've gone ahead and saved this change but I have also included
%the original in the github. 
...
[r,size_s] = size(s);
[size_amp,c] = size(amp_env);

if size_s ~= size_amp
    amp_env = amp_env(1:size_s);
end

toneTwist = s.*amp_env';
toneTwist = toneTwist*(song_rms/rms(toneTwist));
tone = toneTwist';

toneLength = length(tone);
tone(toneLength+1:2000000) = 0;

% each window needs to be 2000000 units, so 2000000 - the length of the
% tone
% e.g. 2000000 - 562440
% silence = zeros(1,(2000000 - num_samp));
...
% 
% fid = fopen('CT18_tone.f32', 'a');         % saved to a new file tone1
